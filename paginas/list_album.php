...
<div id="form-albuns-env">
	<?php
		if(isset($_POST['criaralbum']) AND trim($_POST['tituloalbum'])<>''){
			...
			echo'<script>window.location="album.php?uid='.$idDaSessao.'&aid='.$album.'&ac=ADD_FOTOS";</script>';
...
if($albuns['num']>0){
	echo '<div id="exibe-albuns-principal"><ul>';
	...
	foreach ($albuns['dados'] as $resalbuns) :
		...
		$li = '<li><a href="album.php?uid='.$idExtrangeiro.'&aid='.$resalbuns['id'].'"><img class="capa-album" src="'.(file_exists($file) ? $file : 'uploads/fotos/default.jpg').'" width="120px" height="100px" /><span>'.$resalbuns['titulo'].'  ('.$numfotos['fotos']['num'].')</span></a></li>';
