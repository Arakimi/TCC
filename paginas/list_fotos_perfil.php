...
<span class="voltar-album"><a href="palbum.php?uid=<?php echo $idExtrangeiro; ?>">Voltar para os albuns de <?php echo $user_nome ?></a></span>
<br>

<?php
if(isset($_GET['fid'])){
	include('pgaleria.php');
	exit();
}
?>
<div id="listFotos-perfil">

<?php if(Albuns::meuAlbum((int)$_GET['aid'],$idExtrangeiro) AND $idDaSessao==$idExtrangeiro) {?><div id="btn-adicionar-fotos-album"><a href="palbum.php?uid=<?php echo $idDaSessao ?>&aid=<?php echo (int)$_GET['aid'] ?>&ac=ADD_FOTOS">Adicionar Fotos</a></div><?php } ?>

<?php 
// adicionando fotos
if (isset($_GET['ac']) AND $_GET['ac']=='ADD_FOTOS' AND $idDaSessao==$idExtrangeiro){
	include('paddFotos.php');
	exit();
}
...
if($minhasfotos['fotos']['num']>0){
	...
	foreach ($minhasfotos['fotos']['dados'] as $resfotos):
		echo '<li><a href="palbum.php?uid='.$idExtrangeiro.'&aid='.(int)$_GET['aid'].'&fid='.$resfotos['id'].'" >'.$resfotos['legenda'].'<img src="uploads/fotos/'.$resfotos['foto'].'" width="200" /></a>';
